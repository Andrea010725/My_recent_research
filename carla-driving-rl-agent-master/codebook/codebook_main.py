    ...
    def forward(  # type:ignore
        ...
        if self.cfg.model.codebook.embeds == "joint_embeds":
            ...
            if self.codebook_indexing == "gumbel_softmax":
                codebook_probs = F.gumbel_softmax(self.linear_codebook_indexer(joint_embeds),
                                                  tau=self.cfg.model.codebook.temperature, hard=True, dim=-1)
        ...
        if self.cfg.eval:
            ...
            torch.save(observations['visible_goal_objects'].cpu(),
                       f'{self.pkl_path}/visible_goal_objects_{self.counter}.pt')
            ...
            torch.save(observations['valid_moves_forward'].cpu(),
                       f'{self.pkl_path}/valid_moves_forward_{self.counter}.pt')
...
class VisualNavActorCritic(ActorCriticModel[CategoricalDistr]):
    ...
    def forward(  # type:ignore
        ...
        if self.use_reward_info:
            ...
            reward_weights_embeds = reward_weights_embeds.reshape(reward_weights_embeds.shape[0],
                                                                  reward_weights_embeds.shape[1], -1)  # 1, 2, 72
